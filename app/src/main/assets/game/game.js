(() => {
  ...
  const VALID_LIGHT_MODES = Object.freeze(["auto", "dawn", "day", "dusk", "night"]);
  const DESKTOP_LIGHT_MODES = Object.freeze(["auto", "day", "night"]);
  const VALID_WEATHER_MODES = Object.freeze(["auto", "sunny", "cloudy", "rain", "storm"]);
  ...
  function syncCanvasViewport() {
    ...
    const nextW = clamp(Math.round(H * aspect), 960, 1440);
    if (nextW === W && canvas.width === nextW && canvas.height === H) return;
    W = nextW;
    canvas.width = W;
    canvas.height = H;
  ...
  Object.assign(i18n.zh, {
    readyBadge: "玩法",
    ready: "收集金币，避开尖刺，踏过怪物并触碰旗帜通关；可长按左右移动，同时按住跳跃或冲刺完成连续动作"
  });

  Object.assign(i18n.zhHK, {
    readyBadge: "指引",
    ready: "集金避刺，踏怪触旗以通关；可長按左右移動，同時按住跳躍或衝刺完成連續動作"
  });

  Object.assign(i18n.en, {
    readyBadge: "Guide",
    ready: "Collect Coins, Avoid Thorns, Stomp Enemies, And Touch The Flag. Hold Move While Jumping Or Dashing For Combo Control"
  });

  Object.assign(i18n.fr, {
    readyBadge: "Guide",
    ready: "Ramassez Les Pieces, Evitez Les Epines, Ecrasez Les Ennemis Et Touchez Le Drapeau. Maintenez Le Deplacement Avec Saut Ou Sprint"
  });

  Object.assign(i18n.es, {
    readyBadge: "Guia",
    ready: "Recoge Monedas, Evita Espinas, Pisa Enemigos Y Toca La Bandera. Mantén Movimiento Con Salto O Carrera"
  });

  Object.assign(i18n.ru, {
    readyBadge: "Совет",
    ready: "Собирайте Монеты, Избегайте Шипов, Прыгайте На Врагов И Касайтесь Флага. Удерживайте Движение Вместе С Прыжком Или Рывком"
  });

  Object.assign(i18n.ar, {
    readyBadge: "دليل",
    ready: "اجمع العملات، تجنب الأشواك، اقفز على الأعداء والمس العلم. اضغط الحركة مع القفز أو الاندفاع للتحكم المتصل"
  });

  Object.assign(i18n.ja, {
    readyBadge: "案内",
    ready: "コインを集め、トゲを避け、敵を踏んで旗に触れよう。左右移動を押したままジャンプやダッシュも同時に使える"
  });

  Object.assign(i18n.de, {
    readyBadge: "Guide",
    ready: "Sammle Münzen, Meide Dornen, Stampfe Gegner Und Berühre Die Flagge. Halte Bewegung Mit Sprung Oder Sprint Für Kombosteuerung"
  });
  ...
  const GAME_STATE_MESSAGE_KEYS = [
    "ready", "readyBadge", "start", "paused", "levelClear", "complete", "gameOver", "hurt", "lifeLost",
  ...
  function auditInputCheckpoints() {
    ...
    const touchActionIssues = requiredTouchButtons
      .map((id) => document.getElementById(id))
      .filter(Boolean)
      .filter((button) => getComputedStyle(button).touchAction !== "none")
      .map((button) => button.id);
    const player = makePlayer(0, 0);
    const stateIssues = [];
    if (player.jumpHeld !== false) stateIssues.push("jumpHeldInitialState");
    if (CONTROL_TUNING.jumpBuffer < 0.1) stateIssues.push("jumpBufferTooShortForMobile");
    if (missing.length || touchActionIssues.length || stateIssues.length) {
      console.warn("Star Orchard input checkpoints", { missing, touchActionIssues, stateIssues });
      return false;
    }
    return true;
  }
  ...
  function auditLayoutCheckpoints() {
    // Mobile HUD checks use minimum rendered pixels, not CSS intent, so WebView scaling regressions surface early.
    const layoutLimits = {
      topEdge: 44,
      topGap: 14,
      touchHintFont: 19,
      playHintFont: 22,
      brandFont: 42,
      settingsOptionFont: 19,
      settingsOptionHeight: 50,
      settingsLabelFont: 24,
      settingsSheetWidthRatio: 0.84,
      settingsSheetHeightRatio: 0.76,
      stageMargin: 8
    };
    const ids = ["touchLeft", "touchRight", "touchDash", "touchJump", "systemBtn", "quickRestartBtn", "quickPauseBtn", "quickSaveBtn", "quickLoadBtn", "playHint"];
    ...
    const hasSize = (box) => Boolean(box && box.width > 1 && box.height > 1);
    const gapBetween = (leftRect, rightRect) => rightRect.left - leftRect.right;
    const boxesOverlap = (a, b, gap = 0) => hasSize(a) && hasSize(b)
      && a.left < b.right + gap
      && a.right > b.left - gap
      && a.top < b.bottom + gap
      && a.bottom > b.top - gap;
    const horizontalGap = (a, b) => {
      if (!hasSize(a) || !hasSize(b)) return Infinity;
    ...
    const pauseRect = rect("quickPauseBtn");
    const saveRect = rect("quickSaveBtn");
    const loadRect = rect("quickLoadBtn");
    const startRect = rect("overlayText");
    const playHintRect = rect("playHint");
    const brand = document.querySelector(".brand-mark");
    const brandRect = brand ? brand.getBoundingClientRect() : null;
    const brandFont = brand ? Number.parseFloat(getComputedStyle(brand).fontSize) : 0;
    const playHintFont = playHint ? Number.parseFloat(getComputedStyle(playHint).fontSize) : 0;
    const hintNode = document.querySelector(".touch-hint");
    const hintFont = hintNode ? Number.parseFloat(getComputedStyle(hintNode).fontSize) : 0;
    const systemOption = document.querySelector(".system-option");
    const systemOptionStyle = systemOption ? getComputedStyle(systemOption) : null;
    const systemOptionFont = systemOptionStyle ? Number.parseFloat(systemOptionStyle.fontSize) : 0;
    const systemOptionMinHeight = systemOptionStyle ? Number.parseFloat(systemOptionStyle.minHeight) : 0;
    const systemLabel = document.querySelector(".system-section > span");
    const systemLabelFont = systemLabel ? Number.parseFloat(getComputedStyle(systemLabel).fontSize) : 0;
    const modal = document.querySelector(".system-modal:not(.hidden)");
    const sheet = modal ? document.querySelector(".system-sheet") : null;
    const sheetRect = sheet ? sheet.getBoundingClientRect() : null;
    const missionRect = rect(".mission-dock");
    ...
    const topEdgeMin = compactHud ? layoutLimits.topEdge : 18;
    const topGapMin = compactHud ? layoutLimits.topGap : 6;
    if (hudRect.width > stageRect.width * 0.78) layoutIssues.push("hudTooWide");
    if (compactHud && Math.abs((hudRect.left + hudRect.width / 2) - (stageRect.left + stageRect.width / 2)) > 8) layoutIssues.push("hudNotCentered");
    ...
    if (leftRect && rightRect && gapBetween(leftRect, rightRect) < 8) layoutIssues.push("moveButtonGap");
    if (dashRect && jumpRect && gapBetween(dashRect, jumpRect) < 8) layoutIssues.push("actionButtonGap");
    if (compactHud && statRect && systemRect && Math.abs((systemRect.top + systemRect.height / 2) - (statRect.top + statRect.height / 2)) > 12) layoutIssues.push("topButtonRow");
    if (compactHud && statRect && pauseRect && Math.abs((pauseRect.top + pauseRect.height / 2) - (statRect.top + statRect.height / 2)) > 12) layoutIssues.push("pauseButtonRow");
    if (compactHud && statRect && quickRect && Math.abs((quickRect.top + quickRect.height / 2) - (statRect.top + statRect.height / 2)) > 12) layoutIssues.push("restartButtonRow");
    if (compactHud && leftRect && leftRect.width < 120) layoutIssues.push("moveButtonTooSmall");
    if (compactHud && jumpRect && jumpRect.width < 120) layoutIssues.push("jumpButtonTooSmall");
    if (compactHud && dashRect && dashRect.width < 108) layoutIssues.push("dashButtonTooSmall");
    if (compactHud && startRect && startRect.width > 0 && startRect.height < 96) layoutIssues.push("startButtonTooSmall");
    if (compactHud && startRect && startRect.width > 0 && Math.abs((startRect.left + startRect.width / 2) - (stageRect.left + stageRect.width / 2)) > 8) layoutIssues.push("startButtonNotCentered");
    if (compactHud && hasSize(brandRect) && brandFont < layoutLimits.brandFont) layoutIssues.push("brandTitleTooSmall");
    if (compactHud && systemRect && stageRect.right - systemRect.right < topEdgeMin) layoutIssues.push("topRightEdgeUnsafe");
    if (compactHud && saveRect && saveRect.left - stageRect.left < topEdgeMin) layoutIssues.push("topLeftEdgeUnsafe");
    if (compactHud && leftRect && leftRect.left - stageRect.left < 18) layoutIssues.push("bottomLeftEdgeUnsafe");
    if (compactHud && jumpRect && stageRect.right - jumpRect.right < 18) layoutIssues.push("bottomRightEdgeUnsafe");
    if (compactHud && (Math.abs(stageRect.width - window.innerWidth) > 3 || Math.abs(stageRect.height - window.innerHeight) > 3)) layoutIssues.push("stageNotFullscreen");
    if (compactHud && Math.abs(canvas.width / canvas.height - stageRect.width / stageRect.height) > 0.03) layoutIssues.push("canvasAspectMismatch");
    if (compactHud && statRect && systemRect && (systemRect.height < statRect.height - 2 || systemRect.height > statRect.height * 1.32)) layoutIssues.push("systemHudHeight");
    if (compactHud && statRect && quickRect && (quickRect.height < statRect.height - 2 || quickRect.height > statRect.height * 1.32)) layoutIssues.push("quickHudHeight");
    if (compactHud && boxesOverlap(systemRect, quickRect, 8)) layoutIssues.push("systemRestartOverlap");
    if (compactHud && boxesOverlap(quickRect, pauseRect, 8)) layoutIssues.push("restartPauseOverlap");
    if (compactHud && boxesOverlap(saveRect, loadRect, 8)) layoutIssues.push("saveLoadOverlap");
    if (compactHud && boxesOverlap(hudRect, pauseRect, 8)) layoutIssues.push("hudPauseOverlap");
    if (compactHud && boxesOverlap(hudRect, loadRect, 8)) layoutIssues.push("hudLoadOverlap");
    if (compactHud && horizontalGap(systemRect, quickRect) < topGapMin) layoutIssues.push("systemRestartGap");
    if (compactHud && horizontalGap(quickRect, pauseRect) < topGapMin) layoutIssues.push("restartPauseGap");
    if (compactHud && horizontalGap(saveRect, loadRect) < topGapMin) layoutIssues.push("saveLoadGap");
    if (compactHud && hintFont > 0 && hintFont < layoutLimits.touchHintFont) layoutIssues.push("touchHintTooSmall");
    if (compactHud && playHintFont > 0 && playHintFont < layoutLimits.playHintFont) layoutIssues.push("playHintTooSmall");
    if (compactHud && systemOptionFont > 0 && systemOptionFont < layoutLimits.settingsOptionFont) layoutIssues.push("settingsOptionFontTooSmall");
    if (compactHud && systemOptionMinHeight > 0 && systemOptionMinHeight < layoutLimits.settingsOptionHeight) layoutIssues.push("settingsOptionTooShort");
    if (compactHud && systemLabelFont > 0 && systemLabelFont < layoutLimits.settingsLabelFont) layoutIssues.push("settingsLabelTooSmall");
    if (compactHud && sheetRect && (sheetRect.width < stageRect.width * layoutLimits.settingsSheetWidthRatio || sheetRect.height < stageRect.height * layoutLimits.settingsSheetHeightRatio)) layoutIssues.push("settingsSheetTooSmall");
    if (compactHud && sheetRect && (sheetRect.right > stageRect.right - layoutLimits.stageMargin || sheetRect.left < stageRect.left + layoutLimits.stageMargin || sheetRect.top < stageRect.top + layoutLimits.stageMargin || sheetRect.bottom > stageRect.bottom - layoutLimits.stageMargin)) layoutIssues.push("settingsSheetOutOfBounds");
    if (hasSize(missionRect) && missionRect.left < stageRect.left + layoutLimits.stageMargin) layoutIssues.push("missionDockEdgeClip");
    if (hasSize(playHintRect) && hasSize(touchRect) && playHintRect.bottom + 12 > touchRect.top) layoutIssues.push("playHintTouchOverlap");
    if (hasSize(playHintRect) && boxesOverlap(playHintRect, hudRect, 8)) layoutIssues.push("playHintHudOverlap");
    if (levelOptions) {
  ...
  function normalizeSettings(target = settings) {
    // Saved data may outlive UI changes; keep every persisted mode inside a known, renderable set.
    target.language = i18n[target.language] ? target.language : "zh";
    target.lightMode = VALID_LIGHT_MODES.includes(target.lightMode) ? target.lightMode : "auto";
    target.weatherMode = VALID_WEATHER_MODES.includes(target.weatherMode) ? target.weatherMode : "auto";
    target.maxCleared = Number(target.maxCleared) || 0;
    target.heroName = target.heroName || "Star Orchard";
    target.muted = Boolean(target.muted);
    return target;
  }

  function desktopLightValue() {
    return DESKTOP_LIGHT_MODES.includes(settings.lightMode) ? settings.lightMode : "auto";
  }
  ...
  function showPlayHint(text) {
    ...
    playHint.setAttribute("aria-label", `${t("readyBadge")}: ${text}`);
    const badge = document.createElement("span");
    badge.className = "play-hint-badge";
    badge.setAttribute("aria-hidden", "true");
    const copy = document.createElement("span");
    copy.className = "play-hint-copy";
    copy.textContent = text;
    playHint.append(badge, copy);
    playHint.classList.remove("is-visible");
    playHint.offsetWidth;
    playHint.classList.add("is-visible");
    clearTimeout(playHintTimer);
    playHintTimer = setTimeout(() => playHint.classList.remove("is-visible"), 4800);
  }
  ...
  function announce(text, x = null, y = null, color = "#fff4a8", sound = null) {
    ...
    if (sound === "start") showPlayHint(text);
    else if (x !== null && y !== null) addFloater(x, y, text, color);
  ...
  function syncSystemControls() {
    ...
    if (lightSelect) lightSelect.value = desktopLightValue();
    updateLevelSelect();
  ...
  function loadGame() {
    ...
    normalizeSettings(settings);
    settings.maxCleared = Math.max(settings.maxCleared, Number(data.maxCleared) || 0);
    languageSelect.value = settings.language;
    lightSelect.value = desktopLightValue();
  ...
  function updatePlayer(dt) {
    ...
    const jumpNow = pressed.has("Space") || pressed.has("ArrowUp") || pressed.has("KeyW") || (jumpHeld && !p.jumpHeld);
    ...
    if (!jumpHeld && p.vy < -CONTROL_TUNING.shortHopCutoff * (springActive ? SPRING_JUMP_MULTIPLIER : 1)) p.vy += GRAVITY * 1.7 * dt;
    p.vy += GRAVITY * dt;
    moveAndCollide(p, dt);
  ...
  function bindEvents() {
    const bindTouchButton = (id, code) => {
      ...
      const release = (event) => {
        ...
        button.classList.remove("is-press-flash");
      };
      button.addEventListener("pointerdown", (event) => {
        ...
        pressed.add(code);
        keys.add(code);
        // Split tap flash from held state: quick taps still show feedback, long presses keep the blue ring alive.
        button.classList.remove("is-press-flash");
        button.offsetWidth;
        button.classList.add("is-press-flash");
      ...
      button.addEventListener("lostpointercapture", (event) => {
        // Android WebView can briefly drop pointer capture during two-thumb holds.
        // Treat pointerup/pointercancel as the authoritative release so combo input
        // such as holding Right while pressing Jump does not get cancelled early.
        if (touchPointers.get(event.pointerId) === code) button.classList.add("is-down");
      });
      button.addEventListener("animationend", (event) => {
        if (event.animationName === "controlTapPop") button.classList.remove("is-press-flash");
      });
    ...
    window.addEventListener("orientationchange", () => requestAnimationFrame(syncCanvasViewport));
    window.visualViewport?.addEventListener("resize", syncCanvasViewport);
